...
# the first part of the name of the active object is used as the folder.
# files are saved using the full name of the active object.
path = ""
...
if active.name + "_albedo" in I : albedo_tex = I[active.name + "_albedo"]
else : albedo_tex = bpy.data.images.new(active.name + "_albedo",tex_size[0],tex_size[1])
...
if active.name + "_roughness" in  I : roughness_tex = I[active.name + "_roughness"]
else : roughness_tex = bpy.data.images.new(active.name+"_roughness",tex_size[0],tex_size[1])
...
if active.name + "_normal" in I : normal_tex = I[active.name + "_normal"]
else : normal_tex = bpy.data.images.new(active.name+"_normal",tex_size[0],tex_size[1])
...
folder = active.name.split('_')[0] + '\\'

# if save, save
if save:
    albedo_tex.save_render(filepath=path+folder+active.name+'_albedo.png')
    roughness_tex.save_render(filepath=path+folder+active.name+'_roughness.png')
    normal_tex.save_render(filepath=path+folder+active.name+'_normal.png')
...
if export:
    for object in selected :
        ...
        bpy.ops.export_scene.gltf(
            ...
            filepath = path + folder + object.name,
